...
log_file = open(os.path.join(folder,"stuff.log"),"w")

async def prints(dut,name,i=False):
    ...
    log_file.write(f"prev bit   : {dut.prev_bit.value}\n")
    log_file.write(f"count      : {dut.count.value}\n\n")
    log_file.flush()
...
async def stuffing_test(dut):
    ...
    patterns =[
        ...
        ("Step up 1's", stim.step_up_1(64)),
        ("Step down 1's", stim.step_down_1(64)),
        ("Checkerboard", stim.checkerboard(64)),
